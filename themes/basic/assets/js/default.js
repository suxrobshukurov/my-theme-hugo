var Tickets = {
  initialize: function () {
    ...
    if (typeof window['Sortable'] != 'function') {
      document.write(
        '<script src="' + TicketsConfig.jsUrl + 'lib/sortable/Sortable.min.js"></script>',
      );
      document.write(
        '<script src="' + TicketsConfig.jsUrl + 'lib/sortable/jquery.binding.js"></script>',
      );
    }
    if (!jQuery().ajaxForm) {
    ...
    if (!jQuery().sisyphus) {
      document.write(
        '<script src="' + TicketsConfig.jsUrl + 'lib/jquery.sisyphus.min.js"></script>',
      );
    }
    ...
    $(document).on(
      'click touchend',
      '#ticketForm .preview, #ticketForm .save, #ticketForm .draft, #ticketForm .publish',
      function (e) {
    ...
    $('#comments').on('click touchend', '.ticket-comment-up a', function () {
      ...
      if (parent && id) {
        ...
        $('#comment-' + parent + ' .ticket-comment-down:lt(1)')
          .show()
          .find('a')
          .attr('data-child', id);
      }
  ...
  ticket: {
    ...
    subscribe: function (section) {
      if (section) {
        $.post(
          TicketsConfig.actionUrl,
          { action: 'section/subscribe', section: section },
          function (response) {
            if (response.success) {
              Tickets.Message.success(response.message);
            } else {
  ...
  comment: {
    ...
    getlist: function () {
      ...
      $.post(
        TicketsConfig.actionUrl,
        { action: 'comment/getlist', thread: thread.val() },
        function (response) {
  ...
  author: {
    subscribe: function (author) {
      if (author) {
        $.post(
          TicketsConfig.actionUrl,
          { action: 'author/subscribe', author: author },
          function (response) {
            if (response.success) {
              Tickets.Message.success(response.message);
            } else {
...
Tickets.Vote = {
  comment: {
    ...
    vote: function (link, id, value) {
      ...
      $.post(
        TicketsConfig.actionUrl,
        { action: 'comment/vote', id: id, value: value },
        function (response) {
          if (response.success) {
  ...
  ticket: {
    ...
    vote: function (link, id, value) {
      ...
      $.post(
        TicketsConfig.actionUrl,
        { action: 'ticket/vote', id: id, value: value },
        function (response) {
          if (response.success) {
            link.addClass(options.voted);
            parent.removeClass(options.active).addClass(options.inactive);
            parent.find('.' + options.vote);
            rating
              .text(response.data.rating)
              .attr('title', response.data.title)
              .removeClass(options.vote);
